...
private:
  ...
  void launchHealthCheck(const string& containerName, const TaskInfo& task)
  {
    ...
    // To make sure the health check run in the same file system namespace and
    // network namespace with the Docker container, we wrap the original health
    // check command in `docker exec`.
