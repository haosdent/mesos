...
Future<http::Response> Master::QuotaHandler::get(
    ...
    ContentType contentType) const
{
  ...
  return _status(principal)
    .then([contentType](const QuotaStatus& status) -> http::Response {
      return OK(serialize(contentType, evolve(status)), stringify(contentType));
    });
}
    ...
    const Option<string>& principal) const
{
  ...
  return _status(principal)
    .then([request](const QuotaStatus& status) -> http::Response {
    ...
    const Option<string>& principal) const
{
  ...
  return process::collect(authorizedRoles)
    .then(defer(
        master->self(),
        [=](const list<bool>& authorizedRolesCollected) -> Future<QuotaStatus> {
