...
// command will verify the assumption that the command is run as the
// specified user. We use (and assume the presense) of the
...
TEST_F(SlaveTest, ROOT_RunTaskWithCommandInfoWithUser)
{
  ...
  // Launch a prepare task with root.
  // This prepare task prepare the lt-mesos-executor binary file for next task.
  // Because next task are running with a test user(nobody),
  // the lt-mesos-executor binary file may not exist and
  // could not be generated if run next task directly.
  ...
  // This prepare command executor will run as current user that running test.
  // After this command executor finishs, we could make sure
  // the lt-mesos-executor binary file exists.
